...
void marge(tree &root, tree &L, tree &R, int len);

int n, ar[ 30010 ];
tree stree[30010 * 4], ans;
...
void build(int node, int b, int e)
{
    if(b == e)
    {
        stree[node].mn = stree[node].area = ar[ b ];
        stree[node].range = 1;
        return;
    }

    int left_child = node << 1, right_child = left_child + 1, mid = (b + e) >> 1;
    build(left_child, b, mid);
    build(right_child, mid + 1, e);

    marge(stree[ node ], stree[ left_child ], stree[ right_child ], e - b + 1);
}
void marge(tree &root, tree &L, tree &R, int len)
{
    root.mn = min(L.mn, R.mn);
    root.range = L.range + R.range;
    root.
}

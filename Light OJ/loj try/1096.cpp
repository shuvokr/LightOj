...
void Input()
{
    ...
    while( T-- )
    {
        ...
        mod = 10007;
		CLR(array.ar, 0);
        array.ar[ 0 ][ 0 ] =  a; array.ar[ 0 ][ 1 ] = 0; array.ar[ 0 ][ 2 ] = b; array.ar[ 0 ][ 3 ] = 1; 
		array.ar[ 1 ][ 0 ] = array.ar[ 2 ][ 1 ] = array.ar[ 3 ][ 2 ] = 1;

        array = matrix_expo(array, n - 2); // n - 1 cause prothom pod 0 theke suru, Jodi 1 thake suru hoto then n - 2
        printf("Case %d: %lld\n", ++kag, (((array.ar[ 0 ][ 0 ] * a) % mod) + ((array.ar[ 0 ][ 2 ] * b) % mod) + 
		 ((array.ar[ 0 ][ 3 ] * c) % mod))  % mod);
    }
}
matrix matrix_expo(matrix a, long long p)
{
...
matrix multiply(matrix a, matrix b)
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        {
            long long sum = 0ll;
            for(int k = 0; k < 4; k++) sum += a.ar[ i ][ k ] * b.ar[ k ][ j ];
            ret.ar[ i ][ j ] = sum % mod;
        }
    }
    return ret;

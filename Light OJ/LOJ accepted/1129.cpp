...
void add(trie *a, char *str)
{
    ...
    for(int i = 0; i <= len; i++)
    {
        if(i == len)
        {
            a->word++; // ending of a number
            for(int i = 0; i < 11; i++)
                if(a->mark[ i ] != -1) // after end a word if there is a another edge means a got a number who
                {                       // is prefix of another number
                    flag = 0;
                    return;
                }
        }
        else
        {
            ...
            else
            {
                ...
                if(a->mark[ id ] == -1)
                {
                    a->mark[ id ] = tot++; // new edge created
                    clear_trie(&tri[ a->mark[ id ] ]);
                }
                a = &tri[ a->mark[ id ] ];
            }
        }
